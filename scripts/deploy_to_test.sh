#!/usr/bin/env bash
# 按打包规则把插件部署到主程序测试目录 <apps>/<appid>/。
# 包含: config.yml + logo* + README* + 目标 <版本>/ + 其它非版本子目录(icon/、resources/…)
# 排除: src/、.build.yml、点文件、非目标的版本目录(=含 docker-compose.yml 的其它子目录)
# 用法: deploy_to_test.sh <插件目录> <版本号> [apps目录]
set -euo pipefail

PLUGIN_DIR="${1:?用法: deploy_to_test.sh <插件目录> <版本号> [apps目录]}"
VERSION="${2:?缺少版本号}"
APPS_DIR="${3:-/home/coder/workspaces/dootask/docker/appstore/apps}"
...
for entry in "$PLUGIN_DIR"/*; do
  ...
  # 排除源码目录
  [[ "$base" == "src" ]] && continue
  # 排除「版本目录」(含 docker-compose.yml 的子目录)中非目标的那些
  if [[ -d "$entry" && -f "$entry/docker-compose.yml" && "$base" != "$VERSION" ]]; then
    continue
  fi
  echo "    + $base"
  cp -a "$entry" "$DEST/"
