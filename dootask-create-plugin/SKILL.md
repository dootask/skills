...
常用参照样板（按形态，均为本机绝对路径）：
- 前后端自建镜像型（**默认 JS/TS 全栈首选**）：`/home/coder/workspaces/dootask-plugins/asset-hub`（Next.js + App Router 后端 + SQLite + shadcn，单进程单端口；其 `CLAUDE.md` 与 `docs/rules/` 是权威规范）
...
> 注意布局差异：asset-hub/memos 用 `dootask-plugin/` 包裹布局，本技能新建项目用扁平布局——**照抄它们的 Dockerfile / basePath / nginx / `@dootask/tools` 集成写法，但目录落到 `<appid>/` 扁平结构**（`src/` + `.build.yml` + `<版本>/`）。
...
- **appid / 目录名**：决定 AppStore 应用标识，也是镜像名 `dootask/<appid>` 与 nginx 路径 `/apps/<appid>` 的来源。用小写裸名（如 `approve`、`memos`）；将来要发到社区 AppStore 可用 `community_<作者>_<名>`。默认取英文名小写连字符，向用户确认。
...
把**打包后**的内容（不含 `src/`、`.build.yml`、点文件）拷到主程序应用目录，默认 `/home/coder/workspaces/dootask/docker/appstore/apps/<appid>/`：
...
- `scripts/deploy_to_test.sh` —— 按打包规则把插件部署到主程序 `apps/<appid>/` 测试目录。
