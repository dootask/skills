...
- 应用落地目录（`doo app upload` 导入后所在，只读参考）：`/home/coder/workspaces/dootask/docker/appstore/apps`
...
打包规则（脚本已实现，理解即可）：拷 `config.yml` + `logo*` + `README*` + 目标 `<版本>/` + 其它非版本子目录（如 `icon/`、`resources/`）;**排除** `src/`、`.build.yml`、`.git` 等点文件、以及非目标的版本目录。打成 tar.gz 后调 `doo app upload --appid community_<作者>_<appid>`，后端做合规校验：缺 `config.yml` / `name` 字段非法等会直接报错，不必等到安装时才挂。
...
4. 装坏了的回滚：`doo app uninstall community_<作者>_<appid> [--delete-data]`;**先卸载,再**用 `doo app remove community_<作者>_<appid>` 彻底清掉社区应用的本地目录（直接 remove 已安装应用会被后端拒绝）。
